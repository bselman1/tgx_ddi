...
##' heatmap3
##' Customized version of the heatmap3 function that allows for resizing the header margin.
##' 
...
##' #of statistic tests
##' result$cutTable
heatmap3_custom = function (x, Rowv = NULL, Colv = if (symm) "Rowv" else NULL, 
                    ...
                    verbose = getOption("verbose"),useRaster=if (ncol(x)*nrow(x)>=50000) TRUE else FALSE ,...) 
{
  ...
  # Default height for the top section of the plot containing the legend and plot title
  header_section_height = 0.45
  
  lhei <- c(header_section_height + if (!is.null(main)) 0.2 else 0,4)
